...
public class TwilightForestCompat
{
	...
	private static void registerBlocksItems(CompatChecker checker) {
		...
		if (Compats.isModLoaded("tflostblocks", checker)) {
			DDRegistry.registerDoorBlockAndItem(DDNames.TALL_THORN, DDNames.SHORT_THORN, Properties.of(Material.WOOD).sound(SoundType.WOOD).strength(50.0F, 2000.0F).noOcclusion(), true, DDRegistry.MAIN_TAB);
			DDRegistry.registerDoorBlockAndItem(DDNames.TALL_TOWERWOOD, DDNames.SHORT_TOWERWOOD, Properties.of(Material.WOOD).sound(SoundType.WOOD).strength(40.0F, 6.0F).noOcclusion(), true, DDRegistry.MAIN_TAB);
		}
	}
